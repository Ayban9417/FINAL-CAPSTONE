...
exports.getPanel = async (req, res, next) => {
  ...
  const user = (await queryParamPromise(sql, [req.user]))[0];
  port.write("4");
  res.render('Admin/panel', { user: user, page_name: 'overview' });
};
...
exports.getStart = async (req, res, next) => {
  ...
  const user = (await queryParamPromise(sql, [req.user]))[0];
  port.write("2");
  res.render('Admin/start', { user: user, page_name: 'overview' });
};

exports.getEnd = async (req, res, next) => {
  const sql = 'SELECT * FROM admin WHERE admin_id = ?';
  const user = (await queryParamPromise(sql, [req.user]))[0];
  port.write("3");
  res.render('Admin/end', { user: user, page_name: 'overview' });
};
